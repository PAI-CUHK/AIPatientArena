...
def top_bar():
    with ui.row().style(
        ...
        model_select = ui.select(
            options=["gpt4o", "claude4", "qwen3", "medgemma"],
...
def main_page():
    ...
    ui.markdown(f"### AI 评估结果评价 | 模型：`{model}` | Reviewer ID: `{user_id or '未输入'}` | 已评价: `{len(user_reviews)}`")
...
def review_page(subject_id: str, step: int):
    ...
    if record is None:
        ui.notify(f"⚠️ 未找到 SubjectID={subject_id} 在模型 {model} 中。")
        ui.navigate.to("/")
        return
